...
from ansible2puml.block import get_puml_from_block
from ansible.parsing.dataloader import DataLoader
from ansible2puml.templates import generate_puml
from typing import Optional


def export_playbook(filepath: str, destination: str, render: Optional[bool] = None) -> str:
    file = Path(filepath)
    if not Path(file).is_file():
        exit(f'{filepath} is not a file')
    playbook = Playbook.load(file_name=file.absolute(), loader=DataLoader())
    ...
    graph = generate_puml(puml_string)
    if render:
        pass
    else:
